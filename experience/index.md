# Professional Experience
A structured overview of my professional journey, highlighting roles that blend software engineering, data science, and innovative technology solutions

### **Solution Architect / Software Engineer**
**Arvato Systems GmbH**, Hamburg, Berlin, Germany  
*(June 2023 – Present)*

- Ownership of solution architecture for modular enterprise platforms
- Technical interface between business stakeholders, architecture governance, and delivery teams
- Contribution to distributed system design and cloud-based platform architecture
- Support of scalable DevOps processes and structured delivery models

### **Masterthesis: Data Science**  
**Mercedes-Benz Tech Innovation GmbH**, Stuttgart, Germany  
*(November 2022 – March 2023)*

- Designed data pipelines and machine learning models within ISO 26262 functional safety processes
- Translated analytical findings into structured, decision-support concepts

### **Software Engineer (Working Student)**  
**P&M Agentur Software + Consulting GmbH**, Hamburg, Germany  
*(October 2020 – September 2021)*

- Delivered client-specific software solutions in a consulting-oriented environment
- Led development of internal reservation and project-support tools
- Collaborated directly with stakeholders to translate business requirements into functional systems

### **Bachelorthesis: Software Engineering**  
**P&M Agentur Software + Consulting GmbH**, Hamburg, Germany  
*(June 2021 – August 2021)*

- Prototyped a SaaS-based desk reservation platform following structured development principles

### **Software Engineer (Internship)**  
**P&M Agentur Software + Consulting GmbH**, Hamburg, Germany  
*(March 2021 – May 2021)*

- Developed a tool to support internal project controlling with the Django framework
- Developed and deployed a plugin for digital time tracking on Jira tickets

### **Software Engineer (Internship)**  
**Steinbeis Papier GmbH**, Glückstadt, Germany  
*(January 2020 – November 2020)*

- Developed integrated measurement software for paper thickness
- Communicated with stakeholders to ease production integration

### **Software Engineer (Freelancing)**  
**TPSoftware**, Remote  
*(March 2021 – July 2022)*

- Developed static web pages and Excel automations with macros for labs

### ** Data Collector (Working Student)**  
**Rörd Braren Bereederungs GmbH & Co. KG**, Glückstadt, Germany  
*(November 2018 – January 2020)*

- Captured technical fleet data and provided documentation of fleet components
- Maintained inspection periods of critical parts
